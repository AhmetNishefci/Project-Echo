...
const PLATFORM_CONFIG: Record<Platform, { icon: "logo-instagram" | "logo-snapchat"; color: string; label: string; prefix: string }> = {
  instagram: { icon: "logo-instagram", color: COLORS.primary, label: "Open Instagram", prefix: "@" },
  snapchat: { icon: "logo-snapchat", color: COLORS.snapchat, label: "Open Snapchat", prefix: "" },
};
...
export function SocialActionRow({ platform, handle, onPress }: SocialActionRowProps) {
  ...
  return (
    <TouchableOpacity
      ...
      activeOpacity={0.7}
      accessibilityLabel={`${config.label} ${config.prefix}${handle}`}
      accessibilityRole="button"
    >
      <Ionicons name={config.icon} size={20} color={config.color} style={{ marginRight: 12 }} />
      <View className="flex-1">
        <Text className="text-white text-sm font-semibold">{config.label}</Text>
